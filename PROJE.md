...
17. ~~**Tarayıcıda şifreli defter.**~~ **Tamam.** Tarayıcıda kilit artık zorunlu ve şifrelemenin kendisi: defter bellekte açılıyor, diske yalnızca paroladan türeyen anahtarla mühürlenmiş baytlar yazılıyor. Defter Kimliği ve model anahtarı da sarmalı. Yeni kripto yazılmadı — `veri/kilit.ts`, `sqlite-isci.ts`in `:memory:` yolu ve mühürleme fikri birleştirildi. Şifresiz eski defter kaybolmadan taşınıyor ve düz kopya siliniyor. Gerçek Chromium'da sınandı (`npm run muhur`); tarama önce yalnızca OPFS köküne bakıyordu ve hiçbir şey aramadan geçebilirdi, özyinelemeli yapıldı. `VITE_ONIZLEME` kaldırıldı, K-013 ortadan kalktı (KARARLAR.md · K-037).
